...
if brain_mask.get_shape() != m0.get_shape() or brain_mask.get_shape() != perf.get_shape() or \
...
perf_masked = np.ma.array(perf_data,mask=mask_array)
				...
				#Otherwise run a regression
					...
					if args.nocsf[0] == 1 and np.sum((d_reg>0)) < 2:
						...
						if args.nocsf[0] == 0:
								...
								dwm_pvc_data[dim_1,dim_2,dim_3]] = np.dot(d_reg_inv,perf_kernel)
						else:
							[dgm_pvc_data[dim_1,dim_2,dim_3],
							dwm_pvc_data[dim_1,dim_2,dim_3]] = np.dot(d_reg_inv,perf_kernel)
